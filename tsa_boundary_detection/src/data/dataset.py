...
def infer_dataset_dir_name(path: Path) -> str:
    for part in path.parts:
        if part.endswith('data') and part[:-4].isdigit():
            return part
    return path.parent.name
...
class NPYTransientDataset(Dataset):
    def __init__(
        ...
        label_index: BoundaryLabelIndex,
    ...
    def peek_label(self, idx: int) -> int:
        return int(self.label_index.label_for_path(self.file_list[idx]))
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        boundary_label = int(self.label_index.label_for_path(path))
        original_label = find_first(sample, 'y', 'label')
        ...
        if V_arr.shape[0] != t_arr.shape[0] or delta_arr.shape[0] != t_arr.shape[0]:
            raise ValueError(
                f'time length mismatch for {path}: t={t_arr.shape[0]}, V={V_arr.shape}, delta={delta_arr.shape}'
            )
        ...
        dataset_dir_name = infer_dataset_dir_name(path)
        ...
        return {
            ...
            'boundary_label': torch.tensor(boundary_label, dtype=torch.float32),
            'y': torch.tensor(boundary_label, dtype=torch.float32),
...
def collate_fn(batch: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row, item in enumerate(batch):
        ...
        y[row] = item['boundary_label']

        files.append(item['file'])
        sample_ids.append(item['sample_id'])
        dataset_names.append(item['dataset_name'])
        dataset_dir_names.append(item['dataset_dir_name'])
