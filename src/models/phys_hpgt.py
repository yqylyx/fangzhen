...
def masked_mean(x: Tensor, mask: Tensor, dim: int | tuple[int, ...], keepdim: bool = False) -> Tensor:
    # 统一的带掩码平均池化，时间 patch、通道节点和图节点都会复用它。
    mask_f = mask.float()
...
class ChannelPatchEmbed(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor) -> tuple[Tensor, Tensor]:
        ...
        # 先按通道沿时间切成固定长度 patch，后面的时序建模不再直接面对原始采样点，
        # 而是面对局部时间片段的摘要表示。
        ...
        # 先屏蔽无效时间点，再把每个 patch 投影到统一的 d_model 维度。
...
class GraphAttentionBlock(nn.Module):
    ...
    def _apply_topk(self, scores: Tensor, node_mask: Tensor) -> Tensor:
        if self.graph_type != "sparse_topk":
            return scores
        bsz, n_heads, n_nodes, _ = scores.shape
    ...
    def forward(self, x: Tensor, node_mask: Tensor, return_attn: bool = False) -> tuple[Tensor, Tensor | None]:
        ...
        # 每个通道先得到自己的时序摘要后，再把电压和功角通道当成图节点，
        # 通过图注意力学习跨通道的信息交互关系。
...
class PhysHPGT(nn.Module):
    ...
    def _encode_temporal(
        ...
        if self.temporal_encoder == "transformer":
            # 先把 batch 维和通道维展平，让每个通道先作为一条独立时序来建模，
            # 然后再在后面的图模块里做跨通道交互。
    ...
    def _spectral_feature(self, x: Tensor, channel_mask: Tensor) -> Tensor:
        ...
        # 频域分支补充局部时间 patch 不容易直接表达的振荡信息。
        seq = x.transpose(1, 2)
        if torch.onnx.is_in_onnx_export():
            # ONNX 对 FFT 支持不稳定，这里退化为可导出的时域摘要。
    ...
    def _fuse_modalities(
        ...
        # 分别对电压节点、功角节点和全部节点做汇聚，再学习一个样本级门控，
        # 让模型自己决定当前样本更该依赖哪种模态。
        ...
        gate_input = torch.cat([pooled_v, pooled_d, risk_context], dim=-1)
    ...
    def _resolve_exit_states(self, states: list[Tensor]) -> list[Tensor]:
        ...
        # 不管图模块产生多少个中间决策状态，最终都整理成固定数量的出口。
    ...
    def forward(
        ...
        # 第三步：把每个通道的 patch 序列汇聚成一个通道节点表示。
        ...
        risk_list: list[Tensor] = []
        for idx, state in enumerate(exit_states):
            logits, risk = self.exit_heads[idx](state)
            logits_list.append(logits)
            risk_list.append(risk)

        # 归一化后的嵌入向量会送给监督对比学习损失使用。
